...
async function pocetakZemljista(grad) {
  ...
  const funkcija = async (url) => {
    ...
    if (items.length < 1) {
      ...
      const nextUrl = `https://www.njuskalo.hr/prodaja-zemljista/${grad}?page=${nextPageNumber}`;
  ...
  const firstUrl = `https://www.njuskalo.hr/prodaja-zemljista/${grad}?page=1`;
...
export default pocetakZemljista;