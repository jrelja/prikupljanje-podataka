...
async function pocetakStanovi(grad) {
  ...
  const funkcija = async (url) => {
    ...
    if (items.length < 1) {
      ...
      const nextUrl = `https://www.njuskalo.hr/prodaja-stanova/${grad}?page=${nextPageNumber}`;
  ...
  const firstUrl = `https://www.njuskalo.hr/prodaja-stanova/${grad}?page=1`;
